...
class TabCoordinator: NSObject, Coordinator {
    ...
    func start() {
        ...
        let pages: [TabBarPage] = [.cart, .explore, .home, .profile]
    ...
    private func getTabController(_ page: TabBarPage) -> UINavigationController {
        ...
        case .explore:
            let exploreVC = ExploreScreenViewController()
            exploreVC.didSendEventClosure = { [weak self] event in
                switch event {
                case .explore:
                    self?.selectPage(.explore)
                }
            }
        ...
        case .cart:
            ...
            cartVC.didSendEventClosure = { [weak self] event in
                switch event {
                case .cart:
                    self?.selectPage(.cart)
                }
            }
